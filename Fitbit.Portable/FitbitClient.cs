...
namespace Fitbit.Api.Portable
{
    public class FitbitClient : IFitbitClient
    {
        ...
        /// <param name="encodedUserId"></param>
        /// <returns></returns>
        public async Task<TimeSeriesDataList> GetHeartRateTimeSeriesAsync(TimeSeriesResourceType timeSeriesResourceType, DateTime startDate, DateTime endDate, string encodedUserId = default(string))
        {
            return await GetHeartRateTimeSeriesAsync(timeSeriesResourceType, startDate, endDate.ToFitbitFormat(), encodedUserId);
        }
        ...
        /// <param name="encodedUserId"></param>
        /// <returns></returns>
        private async Task<TimeSeriesDataList> GetHeartRateTimeSeriesAsync(TimeSeriesResourceType timeSeriesResourceType, DateTime baseDate, string endDateOrPeriod, string encodedUserId = default(string))
        {
            var apiCall = FitbitClientHelperExtensions.ToFullUrl("/1/user/{0}/activities/heart/date/{1}/1d/1min.json", encodedUserId, baseDate.ToFitbitFormat());
            ...
            string responseBody = await response.Content.ReadAsStringAsync();
            var serializer = new JsonDotNetSerializer {RootProperty = "activities-heart-intraday" };
            return serializer.GetHeartRateActivityDataList(responseBody);
        }
